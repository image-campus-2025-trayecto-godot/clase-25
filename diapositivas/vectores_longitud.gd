...
func _draw_grid_lines(size: Vector2, center: Vector2):
	...
	while start_x <= size.x:
		grid_lines.draw_line(Vector2(start_x, 0), Vector2(start_x, size.y), Color.GRAY, GRID_LINE_WIDTH)
		start_x += GRID_SPACING
	
	# Draw horizontal grid lines
	var start_y = grid_offset_y
	while start_y <= size.y:
		grid_lines.draw_line(Vector2(0, start_y), Vector2(size.x, start_y), Color.GRAY, GRID_LINE_WIDTH)
		start_y += GRID_SPACING

func _draw_axes(size: Vector2, center: Vector2):
	grid_lines.draw_line(Vector2(center.x, 0), Vector2(center.x, size.y), Color.WHITE, AXIS_LINE_WIDTH)
	grid_lines.draw_line(Vector2(0, center.y), Vector2(size.x, center.y), Color.WHITE, AXIS_LINE_WIDTH)
...
func _draw_coordinate_labels(center: Vector2):
	for i in range(-COORDINATE_RANGE, COORDINATE_RANGE + 1):
		...
		if x_pos >= 0 and x_pos <= grid_lines.size.x:
			grid_lines.draw_string(get_theme_default_font(), Vector2(x_pos - 5, center.y + 20), str(i), HORIZONTAL_ALIGNMENT_CENTER, -1, FONT_SIZE, Color.WHITE)
		
		# Y-axis labels (negative because screen Y is inverted)
		if y_pos >= 0 and y_pos <= grid_lines.size.y:
			grid_lines.draw_string(get_theme_default_font(), Vector2(center.x - 20, y_pos + 5), str(-i), HORIZONTAL_ALIGNMENT_CENTER, -1, FONT_SIZE, Color.WHITE)
...
func _draw_component_lines(vector_end: Vector2):
	if abs(vector_x) > MIN_VECTOR_THRESHOLD:
		vector_arrow.draw_line(Vector2.ZERO, Vector2(vector_end.x, 0), Color.CYAN, COMPONENT_LINE_WIDTH)
	if abs(vector_y) > MIN_VECTOR_THRESHOLD:
		vector_arrow.draw_line(Vector2(vector_end.x, 0), vector_end, Color.CYAN, COMPONENT_LINE_WIDTH)

func _draw_main_vector(vector_end: Vector2):
	vector_arrow.draw_line(Vector2.ZERO, vector_end, Color.RED, VECTOR_LINE_WIDTH)
...
func _draw_arrowhead(vector_end: Vector2):
	...
	vector_arrow.draw_line(vector_end, arrow_point1, Color.RED, VECTOR_LINE_WIDTH)
	vector_arrow.draw_line(vector_end, arrow_point2, Color.RED, VECTOR_LINE_WIDTH)
...
func _draw_right_angle_indicator(vector_end: Vector2):
	if abs(vector_x) > MIN_VECTOR_THRESHOLD and abs(vector_y) > MIN_VECTOR_THRESHOLD:
		...
		vector_arrow.draw_polyline(corner_points, Color.YELLOW, COMPONENT_LINE_WIDTH)
